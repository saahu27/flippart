...
NistCompetitor::NistCompetitor()
    : Node("nist_competitor"),
      ...
      ceiling_wrist_(std::shared_ptr<rclcpp::Node>(std::move(this)), "ceiling_wrist"),
      planning_scene_()
{
  ...
  ceiling_arm_.setMaxAccelerationScalingFactor(1.0);
  ceiling_arm_.setMaxVelocityScalingFactor(1.0);
  ceiling_arm_.setPlanningTime(10.0);
  ceiling_arm_.setNumPlanningAttempts(5);
  ceiling_arm_.allowReplanning(true);
  ceiling_arm_.setReplanAttempts(5);

  ceiling_wrist_.setMaxAccelerationScalingFactor(1.0);
  ceiling_wrist_.setMaxVelocityScalingFactor(1.0);
  ceiling_wrist_.setPlanningTime(10.0);
  ceiling_wrist_.setNumPlanningAttempts(5);
  ceiling_wrist_.allowReplanning(true);
  ceiling_wrist_.setReplanAttempts(5);
  // Subscribe to topics
  ...
  timer_ = this->create_wall_timer(
    ...
    std::bind(&NistCompetitor::PublishTransform, this));

  RCLCPP_INFO(this->get_logger(), "Initialization successful.");
}

void NistCompetitor::PublishTransform()
{
  auto torso_tray_pose = FrameWorldPose("torso_tray");
  double traywidth = 0.77;
  geometry_msgs::msg::PoseStamped target_pose ;
  auto tray_rotation = GetYaw(torso_tray_pose);

  KDL::Frame world_to_tray;
  tf2::fromMsg(torso_tray_pose, world_to_tray);

  double tray_link_offset = 0.0096;
  KDL::Frame tray_to_part;
  tf2::fromMsg(BuildPose(0, - traywidth/2 + 0.02, tray_link_offset+ part_heights_[ariac_msgs::msg::Part::SENSOR], QuaternionFromRPY(M_PI, 0, 0)), tray_to_part);
  ...
  tray_to_part_msg.header.stamp= this->now();
  tray_to_part_msg.header.frame_id= "torso_tray";
  tray_to_part_msg.child_frame_id = "tray_to_part";
  tray_to_part_msg.transform.translation.x = tf2::toMsg(tray_to_part).position.x;
  tray_to_part_msg.transform.translation.y = tf2::toMsg(tray_to_part).position.y;
  tray_to_part_msg.transform.translation.z = tf2::toMsg(tray_to_part).position.z;
  tray_to_part_msg.transform.rotation = tf2::toMsg(tray_to_part).orientation;
  transform_broadcaster_->sendTransform(tray_to_part_msg);

  geometry_msgs::msg::TransformStamped part_to_gripper_msg;
  part_to_gripper_msg.header.stamp= this->now();
  part_to_gripper_msg.header.frame_id= "tray_to_part";
  part_to_gripper_msg.child_frame_id = "part_to_gripper";
  part_to_gripper_msg.transform.translation.x = tf2::toMsg(part_to_gripper).position.x;
...
void NistCompetitor::CeilingArmSendHome()
{
  ceiling_arm_.setJointValueTarget(ceiling_arm_tray_js_);
  CeilingArmMovetoTarget();
}
...
bool NistCompetitor::CeilingArmMovetoTarget()
{
  moveit::planning_interface::MoveGroupInterface::Plan plan;
  bool success = static_cast<bool>(ceiling_arm_.plan(plan));

  if (success)
  {
    robot_trajectory::RobotTrajectory rt(ceiling_arm_.getCurrentState()->getRobotModel(), "ceiling_arm");
    rt.setRobotTrajectoryMsg(*ceiling_arm_.getCurrentState(), plan.trajectory);
    totg_.computeTimeStamps(rt, 0.5, 0.5);
    rt.getRobotTrajectoryMsg(plan.trajectory);
    return static_cast<bool>(ceiling_arm_.execute(plan));
  }
  else
  {
    RCLCPP_ERROR(get_logger(), "Unable to generate plan");
...
bool NistCompetitor::CeilingArmMoveCartesian(
    std::vector<geometry_msgs::msg::Pose> waypoints, double vsf, double asf, bool avoid_collisions)
{
  moveit_msgs::msg::RobotTrajectory trajectory;

  double path_fraction = ceiling_arm_.computeCartesianPath(waypoints, 0.01, 0.0, trajectory, avoid_collisions);

  RCLCPP_INFO(get_logger(), trajectory.joint_trajectory.header.frame_id.c_str());
  if (path_fraction < 0.9)
  {
    RCLCPP_ERROR(get_logger(), "Unable to generate trajectory through waypoints");
    return false;
  ...
  // Retime trajectory
  robot_trajectory::RobotTrajectory rt(ceiling_arm_.getCurrentState()->getRobotModel(), "ceiling_arm");
  rt.setRobotTrajectoryMsg(*ceiling_arm_.getCurrentState(), trajectory);
  totg_.computeTimeStamps(rt, vsf, asf);
  rt.getRobotTrajectoryMsg(trajectory);

  return static_cast<bool>(ceiling_arm_.execute(trajectory));
}

bool NistCompetitor::CeilingWristMoveCartesian(
    std::vector<geometry_msgs::msg::Pose> waypoints, double vsf, double asf, bool avoid_collisions)
{
  moveit_msgs::msg::RobotTrajectory trajectory;

  double path_fraction = ceiling_wrist_.computeCartesianPath(waypoints, 0.01, 0.0, trajectory, avoid_collisions);
  ...
  // Retime trajectory
  robot_trajectory::RobotTrajectory rt(ceiling_wrist_.getCurrentState()->getRobotModel(), "ceiling_wrist");
  rt.setRobotTrajectoryMsg(*ceiling_wrist_.getCurrentState(), trajectory);
  totg_.computeTimeStamps(rt, vsf, asf);
  rt.getRobotTrajectoryMsg(trajectory);

  return static_cast<bool>(ceiling_wrist_.execute(trajectory));
}
...
bool NistCompetitor::CeilingRobotPickAGVPart(ariac_msgs::msg::PartPose part)
{
  ...
  waypoints.push_back(BuildPose(part.pose.position.x + dx, part.pose.position.y + dy,
                                part.pose.position.z + pick_offset_, SetRobotOrientation(part_rotation)));
...
//flip part on torso tray
  waypoints.clear();

  ceiling_arm_.setJointValueTarget(ceiling_arm_tray_);
  auto x = CeilingArmMovetoTarget();

  // CeilingArmSendHome();

  // auto currentpose = ceiling_arm_.getCurrentPose("ceiling_gripper");

  // waypoints.clear();
  // waypoints.push_back(BuildPose(currentpose.pose.position.x, currentpose.pose.position.y,
  //                               currentpose.pose.position.z,
  //                               SetRobotOrientation(-3.14/2)));

  // CeilingWristMoveCartesian(waypoints, 0.3, 0.3, true);


  // CeilingArmMoveCartesian(waypoints, 0.3, 0.3, true);
  // CeilingRobotMoveToAssemblyStation(ariac_msgs::msg::AssemblyTask::AS1);
  ...
  double tray_link_offset = 0.0096;
  KDL::Frame tray_to_part;
  tf2::fromMsg(BuildPose(0, - traywidth/2 + 0.02, tray_link_offset + part_heights_[part.part.type], QuaternionFromRPY(M_PI, 0, 0)), tray_to_part);
  ...
  // geometry_msgs::msg::Pose starting_pose = ceiling_arm_.getCurrentPose().pose;

  // starting_pose.position.y = starting_pose.position.y + 0.4;
  ...
  waypoints.clear();
  waypoints.push_back(tf2::toMsg(world_to_tray * tray_to_part * part_to_gripper * KDL::Frame(away1)));

  waypoints.push_back(tf2::toMsg(world_to_tray * tray_to_part * part_to_gripper * KDL::Frame(away2)));
  ...
  // KDL::Frame tray_to_part;
  // tf2::fromMsg(BuildPose(0, - traywidth/2 + 0.02, torso_tray_pose.position.z, QuaternionFromRPY(0, M_PI/2, 0)), tray_to_part);

  // KDL::Frame partbase_to_sidepick;
  // tf2::fromMsg(BuildPose(-0.015, 0.035, 0.035, QuaternionFromRPY(-M_PI/2, 0, 0)), partbase_to_sidepick);

  // KDL::Frame world_to_torsotray;
  // tf2::fromMsg(torso_tray_pose, partbase_to_sidepick);

  // KDL::Frame pickframe = world_to_torsotray * tray_to_part * partbase_to_sidepick;

  // auto pickpose = tf2::toMsg(pickframe);

  // waypoints.clear();
  // KDL::Vector away(0, 0, 0.1);
  // waypoints.push_back(tf2::toMsg(world_to_torsotray * tray_to_part * partbase_to_sidepick * KDL::Frame(away)));
  // waypoints.push_back(tf2::toMsg(world_to_torsotray * tray_to_part * partbase_to_sidepick));
